...
######################################################################################################################
''' 
This code is used to scrap data from the bestbuy website. One can adapt it to scrap data for his own purpose
'''
...
def URL(s, t):
    url = 'https://www.bestbuy.ca/en-ca/category/laptops-macbooks/20352.aspx?type=product&page=' + s + \
          '&filter=category%253aComputers%2B%2526%2BTablets%253bcategory%253aLaptops%2B%2526%2BMacBooks%253bcustom0ramsize%253a8' + t
    return url
...
def main(csvfile):
    ...
    for t in Ram_url:
        ...
        for s in pages:
            ...
            laptops_containers = page_html.find_all(
                'div', class_='item-inner clearfix')
...
def cleaning(df):
    ...
    df['prices'] = [clean_price(x) for x in df['prices']]
...
if __name__ == '__main__':
    csvfile = 'laptops_rating2020.csv'
    main()
